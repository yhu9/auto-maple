...
def dirty(func):
    """Decorator function that sets the dirty bit for mutative Routine
    operations."""
    ...
    def load(self, file=None):
        ...
        if not file:
            if self.path:
                file = self.path
                print(
                    " *  File path not provided, using previously loaded routine"
                )
            else:
                print(
    ...
    def _eval(self, row, i):
        if row and isinstance(row, list):
            ...
            try:
                ...
                print(
                    line_error + f"Found invalid arguments for '{c.__name__}':"
                )
                print(f"{' ' * 4} -  {e}")
