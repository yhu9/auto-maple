...
class Minimap(LabelFrame):
    ...
    def display_minimap(self):
        ...
        if minimap:
            ...
            if rune_active:
                cv2.circle(
                    img, utils.convert_to_absolute(rune_pos, img), 3, (128, 0, 128), -1
                )
            ...
            for p in config.routine.sequence:
                if isinstance(p, Point):
                    utils.draw_location(
                        img, p.location, (0, 255, 0) if config.enabled else (255, 0, 0)
                    )
            ...
            cv2.circle(
                img, utils.convert_to_absolute(player_pos, img), 3, (0, 0, 255), -1
            )
