...
from src.gui.interfaces import Frame, LabelFrame, MenuBarItem
...
class Update(MenuBarItem):
    def __init__(self, parent, **kwargs):
        ...
        for path in config.bot.submodules:
            ...
            self.add_command(label=name, command=lambda: UpdatePrompt(self, name, path))
...
class UpdatePrompt(tk.Toplevel):
    ...
    def __init__(self, parent, name, path, **kwargs):
        ...
        self.refresh = tk.Button(
            controls_frame, text="Refresh", command=self._refresh_display
        )
        self.refresh.pack(side=tk.TOP, pady=(10, 5))
        self.soft_update = tk.Button(
            controls_frame, text="Update", command=self._update
        )
        self.soft_update.pack(side=tk.BOTTOM)
        self.force_update = tk.Button(
            controls_frame, text="Rebuild", command=lambda: self._update(force=True)
        )
        self.force_update.pack(side=tk.BOTTOM, pady=(0, 5))

        self.listbox.bindtags(
            (self.listbox, config.gui.root, "all")
        )  # Unbind all events
    ...
    def _refresh_display(self):
        ...
        diffs = []
        paths = set()
        for item in repo.index.diff(None) + repo.index.diff(
            "HEAD"
        ):  # Unstaged and staged changes
            path = item.a_path
            if (
                path not in paths
            ):  # Only show changes once per file, unstaged has precedence over staged
