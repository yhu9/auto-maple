...
def merge_detection(model, image):
    ...
    if (
        len(boxes) == 4
    ):  # Only run further inferences if arrows have been correctly detected
        ...
        preprocessed = np.full(
            (pad_height, pad_width, channels), (0, 0, 0), dtype=np.uint8
        )
        ...
        rotated_classes = [
            converter[label_map[item[2]]] for item in lst if item[2] in [1, 2]
        ]
